...
/**
 * <p> Tests related to tspi:UnnumberedThoroughfareAddress elements </p>
 * 
 * @author Brian Uri!
 * @since 2.2.0
 */
public class UnnumberedThoroughfareAddressTest extends AbstractBaseTestCase {
	 ...
	 * Constructor
	 */
	public UnnumberedThoroughfareAddressTest() {
		super("unnumberedThoroughfareAddress.xml");
	 ...
	 */
	public static UnnumberedThoroughfareAddress getFixture() {
		try {
			UnnumberedThoroughfareAddress.Builder builder = new UnnumberedThoroughfareAddress.Builder();
	 ...
	 */
	private UnnumberedThoroughfareAddress getInstance(Element element, String message) {
		boolean expectFailure = !Util.isEmpty(message);
		UnnumberedThoroughfareAddress component = null;
		try {
			component = new UnnumberedThoroughfareAddress(element);
	 ...
	 * @return a valid object
	 */
	private UnnumberedThoroughfareAddress getInstance(UnnumberedThoroughfareAddress.Builder builder, String message) {
		boolean expectFailure = !Util.isEmpty(message);
		UnnumberedThoroughfareAddress component = null;
		try {
	 ...
	 */
	private UnnumberedThoroughfareAddress.Builder getBaseBuilder() {
		DDMSVersion version = DDMSVersion.getCurrentVersion();
		UnnumberedThoroughfareAddress component = getInstance(getValidElement(version.getVersion()), SUCCESS);
		return (new UnnumberedThoroughfareAddress.Builder(component));
	}
	...
	private static String getExpectedXMLOutput() {
		...
		xml.append("<tspi:UnnumberedThoroughfareAddress ");
		...
		xml.append("<addr_type:CountryName>ge:GENC:n:ed1:840</addr_type:CountryName>");
		xml.append("</tspi:UnnumberedThoroughfareAddress>");
		return (xml.toString());
	}
	...
	public void testNameAndNamespace() {
		for (String sVersion : getSupportedVersions()) {
			...
			assertNameAndNamespace(getInstance(getValidElement(sVersion), SUCCESS), DEFAULT_TSPI_PREFIX,
				UnnumberedThoroughfareAddress.getName(version));
	...
	public void testConstructorsMinimal() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			// No optional fields
			UnnumberedThoroughfareAddress.Builder builder = getBaseBuilder();
	...
	public void testValidationWarnings() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			// No warnings
			UnnumberedThoroughfareAddress component = getInstance(getValidElement(sVersion), SUCCESS);
	...
	public void testEquality() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			UnnumberedThoroughfareAddress builderComponent = new UnnumberedThoroughfareAddress.Builder(elementComponent).commit();
