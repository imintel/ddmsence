...
/**
 * <p> Tests related to tspi:NumberedThoroughfareAddress elements </p>
 * 
 * @author Brian Uri!
 * @since 2.2.0
 */
public class NumberedThoroughfareAddressTest extends AbstractBaseTestCase {
	 ...
	 * Constructor
	 */
	public NumberedThoroughfareAddressTest() {
	 ...
	 */
	private NumberedThoroughfareAddress getInstance(Element element, String message) {
		boolean expectFailure = !Util.isEmpty(message);
		NumberedThoroughfareAddress component = null;
		try {
			component = new NumberedThoroughfareAddress(element);
	 ...
	 * @return a valid object
	 */
	private NumberedThoroughfareAddress getInstance(NumberedThoroughfareAddress.Builder builder, String message) {
		boolean expectFailure = !Util.isEmpty(message);
		NumberedThoroughfareAddress component = null;
		try {
	 ...
	 */
	private NumberedThoroughfareAddress.Builder getBaseBuilder() {
		DDMSVersion version = DDMSVersion.getCurrentVersion();
		NumberedThoroughfareAddress component = getInstance(getValidElement(version.getVersion()), SUCCESS);
		return (new NumberedThoroughfareAddress.Builder(component));
	}
	...
	private String getExpectedOutput(boolean isHTML) throws InvalidDDMSException {
		...
		text.append(buildOutput(isHTML, "addressType", "NumberedThoroughfareAddress"));
	...
	private static String getExpectedXMLOutput() {
		...
		xml.append("<addr_type:CountryName>ge:GENC:2:ed1:US</addr_type:CountryName>");
		xml.append("</tspi:NumberedThoroughfareAddress>");
		return (xml.toString());
	}
	...
	public void testNameAndNamespace() {
		for (String sVersion : getSupportedVersions()) {
			...
			assertNameAndNamespace(getInstance(getValidElement(sVersion), SUCCESS), DEFAULT_TSPI_PREFIX,
				NumberedThoroughfareAddress.getName(version));
	...
	public void testConstructorsMinimal() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			// No optional fields
			NumberedThoroughfareAddress.Builder builder = getBaseBuilder();
	...
	public void testValidationWarnings() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			// No warnings
			NumberedThoroughfareAddress component = getInstance(getValidElement(sVersion), SUCCESS);
	...
	public void testEquality() throws InvalidDDMSException {
		for (String sVersion : getSupportedVersions()) {
			...
			// Base equality
			NumberedThoroughfareAddress elementComponent = getInstance(getValidElement(sVersion), SUCCESS);
			NumberedThoroughfareAddress builderComponent = new NumberedThoroughfareAddress.Builder(elementComponent).commit();
